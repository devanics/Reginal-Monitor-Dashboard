import type { MapLayers, LayerDefinition, MapRenderer, MapVariant } from '@/types';
...
export function bindLayerSearch(container: HTMLElement): void {
  ...
  searchInput.addEventListener('input', () => {
    ...
    container.querySelectorAll('.layer-toggle').forEach(label => {
      ...
      const match = text.includes(q) || key.toLowerCase().includes(q);
